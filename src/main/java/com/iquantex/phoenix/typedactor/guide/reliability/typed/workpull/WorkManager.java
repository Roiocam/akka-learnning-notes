...
public class WorkManager extends AbstractBehavior<ImgWorkMessage> {
    ...
    public WorkManager(
            ActorContext<ImgWorkMessage> context, StashBuffer<ImgWorkMessage> stashBuffer) {
        super(context);
        this.ctx = context;
    ...
    public static Behavior<ImgWorkMessage> create() {
        return Behaviors.setup(
                ctx -> {
                    // 消息转换
                    ActorRef<RequestNext<ConversionJob>> messageAdapter =
                            ctx.messageAdapter(
                                    WorkPullingProducerController.requestNextClass(),
                                    WrapperRequestNext::new);
                    // 创建 Controller 并启动
                    ActorRef<WorkPullingProducerController.Command<ConversionJob>>
                            producerController =
                                    ctx.spawn(
                    ...
                    return Behaviors.withStash(1000, stash -> new WorkManager(ctx, stash));
                });
    }
